...
<script>
  $(document).ready(function(){
	...
	<%
		...
		String jsql = "select count(*) from noticeList";
		...
		String jsql2 = "select * from noticeList order by ymd desc";
	...
	%>
      ...
      <div class="container06">
	      <hr style="border: solid 1px lightgray;"><br>
      		...
      		<ul class="tab_wrap">
      			<a href="customNotice.jsp"><li style="color: #000;border-bottom:2px solid #111;"><img class="noti" src="images/notice1.png">공지사항</li></a>
	...
	if (!rs.wasNull()) 
	{
       ...
       while (rs.next()) 
	   {
			  ...
			  String subject = rs.getString("noticeSubject");
              String ymd = rs.getString("ymd").substring(0, 10);
    ...
    <tr >
 	      ...
 	      <td>
 	         <a href="customNoticeView.jsp?no=<%=no%>"><font size=2><%=title%></font></a>
 	      </td> 	      
 	      <td>
 	         <div align="center"><font size="2"><%=ymd%></font></div>
 	      </td> 	      
 	      <td>
   ...
   %>
   	...
   	<table style="margin: auto;margin-top: 56px;">
		...
		%>
        <%= "["%><a href="customNotice.jsp?group_index=0"><font size=2>처음</font></a><%="]" %>
        &nbsp;&nbsp;&nbsp;
   ...
   <%
			...
			{
   %>
        <a href="customNotice.jsp?group_index=<%= j - 1 %>"><%= j %></a>
   <%
            }
       }
   %>
	  ...
	  <%
	   ...
	   %>
      <%= "["%><a href="customNotice.jsp?group_index=<%= cntList_1 - 1 %>"><font size=2>끝</font></a><%="]" %>
    			</div>
             </td>
          </tr>
    </table>
