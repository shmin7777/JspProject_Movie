		...
		if(rs3.next()){
			...
			if(!payPhone.equals("")&&!payPhone.equals(userPhone)){
				...
				while(rs.next()){
					...
					String jsql = "select snackCtg,price from snack where snack_no=?";
					PreparedStatement jpstmt = con.prepareStatement(jsql);
					jpstmt.setInt(1, snack_no);
					ResultSet jrs = jpstmt.executeQuery();
					jrs.next();
					String snackCtg= jrs.getString(1);
					int snackPrice = jrs.getInt(2);
					...
					if(snackCtg.equals("ticket")){
						...
						pstmt2.setString(4,"사용");
						pstmt2.executeUpdate();
						
						String jsql2 = "update usergrade set ticket=? where id=?";
						PreparedStatement jpstmt2 = con.prepareStatement(jsql2);
						jpstmt2.setInt(1, snackQty+ticket);
						jpstmt2.setString(2, id);
						jpstmt2.executeUpdate();
					}else{
						String sql2 = "insert into reserveSnack values(null,?,?,?,?)";
						PreparedStatement pstmt2 = con.prepareStatement(sql2);
						pstmt2.setString(1, id);
